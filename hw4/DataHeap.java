...
class DataHeap extends DataBT implements IHeap {
	...
	public IHeap merge(IHeap withHeap) {
		...
		// we'd rather not use instanceOf here, but it makes the merge algorithm
		// easier to see
		if (withHeap instanceof MtHeap) {
			return this;
		}
		// determine the new root value and the three subtrees to consider
		// merging
		...
		if (H1.height() > H2.height() && H1.height() > H3.height()) {
			...
			// If the two bigger heaps are of the same size, choose one at
			// random.
	...
	// returns the left piece of data as a IHeap
	@Override
	public IHeap getLeft() {
		return this.left;
	}

	// returns the right piece of data as a IHeap
	@Override
